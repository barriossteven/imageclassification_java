...
public class Perceptron {
	...
	public Perceptron(int labelType, int iterations,double percent) throws NumberFormatException, IOException {
		if (labelType == 0) {
			lineLength = 60;
			imageLength = 70;
	...
	public void adjustWeights(int correct, int guessed, char[][] features) {
		...
		for (int j = 0; j < weights[correct].length; j++) {
			for (int k = 0; k < weights[correct][j].length; k++) {
				...
				weights[correct][j][k] = weights[correct][j][k] + currChar;// adjustment
				weights[guessed][j][k] = weights[guessed][j][k] - currChar;// adjustment
			}
		}
	...
	public void validate(String Labels, String Images ) throws IOException {
		...
		if(testing){
			System.out.println("Perceptron Success Rate: " + String.format("%.2f", 100*(v_correct/totalLabels)) + "%");
